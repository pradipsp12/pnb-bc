      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isGiven ? 'bg-red-100' : 'bg-green-100'}`}>
          ...
          {isEdit && (
            <div className="flex rounded-xl border border-gray-200 overflow-hidden">
              <button onClick={() => setType('given')}
                className={`flex-1 py-2 text-sm font-semibold transition-colors ${type === 'given' ? 'bg-red-500 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
                💸 Given
              </button>
              <button onClick={() => setType('received')}
                className={`flex-1 py-2 text-sm font-semibold transition-colors ${type === 'received' ? 'bg-green-500 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
                💰 Received
              </button>
            </div>
          )}
        ...
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button onClick={handleSubmit} disabled={saving}
            className={`flex-1 py-2.5 rounded-xl text-white text-sm font-semibold transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2
              ${isGiven ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}>
...
function Bubble({ tx, runningBalance, onEdit, onDelete }) {
  ...
  return (
    <div className={`flex ${isGiven ? 'justify-end' : 'justify-start'} mb-3 px-4`}>
      <div className="relative max-w-[78%] sm:max-w-[60%] group">
        <div className={`rounded-2xl px-4 py-3 shadow-sm
          ${isGiven ? 'bg-red-500 text-white rounded-tr-sm' : 'bg-white border border-gray-200 text-gray-900 rounded-tl-sm'}`}>

          <div className={`text-xs font-bold mb-1 ${isGiven ? 'text-red-200' : 'text-gray-400'}`}>
            {isGiven ? '💸 Given' : '💰 Received'}
          </div>
          <div className={`text-xl font-extrabold tracking-tight ${isGiven ? 'text-white' : 'text-gray-900'}`}>
            {fmt(tx.amount)}
          </div>
          {tx.note && (
            <div className={`text-xs mt-1.5 leading-relaxed ${isGiven ? 'text-red-100' : 'text-gray-500'}`}>
              {tx.note}
            </div>
          )}
          <div className={`text-xs mt-2 ${isGiven ? 'text-red-200' : 'text-gray-400'}`}>
        ...
        <div className={`text-xs mt-1 px-1 font-medium
          ...
          Balance: {runningBalance === 0 ? 'Settled ✅' : runningBalance > 0 ? `Due ${fmt(runningBalance)}` : `Advance ${fmt(runningBalance)}`}
        </div>

        {/* 3-dot menu */}
        <div ref={menuRef} className={`absolute top-2 ${isGiven ? 'left-2' : 'right-2'}`}>
          <button onClick={() => setMenuOpen(v => !v)}
            className={`p-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity
              ${isGiven ? 'hover:bg-red-400 text-red-200' : 'hover:bg-gray-100 text-gray-400'}`}>
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/>
            </svg>
          </button>
          {menuOpen && (
            <div className={`absolute top-7 ${isGiven ? 'left-0' : 'right-0'} bg-white rounded-xl shadow-xl border border-gray-100 z-20 min-w-[130px] overflow-hidden`}>
...
export default function TransactionClient({
  ...
  const handleSaved = (tx, newBalance, isEdit) => {
    if (isEdit) {
      ...
      // New tx appended at end — always visible (visibleFrom stays, end index grows)
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-screen bg-gray-100 overflow-hidden">
        ...
        {/* ── Chat area ── */}
        <div className="flex-1 overflow-y-auto pb-4 pt-3 max-w-2xl mx-auto w-full">
        ...
        <div className="flex-shrink-0 bg-white border-t border-gray-200 z-20">
          <div className="max-w-2xl mx-auto px-4 py-3 flex gap-3">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-red-500 hover:bg-red-600 active:bg-red-700 text-white font-bold text-sm transition-colors shadow-sm">
